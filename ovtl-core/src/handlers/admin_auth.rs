...
/// Accept any of:
/// 1. X-OVTL-Admin-Key header matching the configured key.
/// 2. Bearer JWT from the master tenant.
/// 3. Bearer JWT whose `realm_access.roles` contains "SuperAdmin" and whose
///    `tid` matches the `x-ovtl-tenant-id` header (tenant-scoped admin).
///
...
pub fn require_admin(
    ...
    //    If x-ovtl-tenant-id header is present it must match the JWT's tid (blocks cross-tenant
